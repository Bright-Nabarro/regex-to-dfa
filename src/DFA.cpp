...
DFA::DFA(const RegexTree& tree)
{
    ...
    for (std::size_t i = 0; i < dstates.size(); ++i)
    {
        ...
        if (std::any_of(dstates[i].followpos.cbegin(),
                        dstates[i].followpos.cend(),
        ...
        for (auto character : alphabet)
        {
            ...
            for (auto next_pos : dstates[i].followpos)
            {
                if (tree.CharAtPos(character, next_pos))
                {
                    ...
                    new_next_positions.insert(followpos.cbegin(),
                                              followpos.cend());
                }
            }

            if (new_next_positions.empty())
                continue;
            ...
            if (old_dstate == dstates.end())
            {
                ...
                dstates[i].state->AddTransition(dstates.back().state.get(),
                                                character);
            }
            else
            {
                dstates[i].state->AddTransition(old_dstate->state.get(),
                                                character);
            }
        }
    }
